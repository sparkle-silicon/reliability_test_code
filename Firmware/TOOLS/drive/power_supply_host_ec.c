...
EXPORT_SYMBOL(ec_read_all);
    ...
    static void loongson_power_info_power_status_update(void)
    {
        ...
        power_info->adapter_in = (power_status & MASK(BIT_POWER_ACPRES)) ?
    ...
    static int thread_read_ecinfo(void *data) {
        ...
        do {
            ...
            //power_info->adapter_in = (unsigned short)ec_read_all(CMD_READ_EC, INDEX_POWER_STATUS);
                ...
                enum power_supply_property psp, union power_supply_propval * val)
    {
        switch (psp) {
            case POWER_SUPPLY_PROP_ONLINE:
                            //loongson_power_info_power_status_update(); //wzk_debug_interface
                val->intval = power_info->adapter_in;
                break;
            default:
                return -EINVAL;
        }
