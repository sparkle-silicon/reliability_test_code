...
int send_file_data(int fd, const char *file_path)
{
    ...
    // 如果文件大小不是256的倍数，进行256字节对齐并补充0xFF
    char *file_data = NULL;
    unsigned int crc32 = 0;
    if(file_size % 256 != 0)
    {
        long aligned_size = file_size + (256 - (file_size % 256));  // 计算对齐后的大小
    ...
    //发送开始进行更新的命令
    char update_cmd[17] = { 0x75 ,0x70 ,0x64 ,0x61 ,0x74 ,0x65 ,0x20 ,0x66 ,0x69 ,0x6d ,0x77 ,0x61 ,0x72 ,0x65 ,0x0D ,0x0A };
